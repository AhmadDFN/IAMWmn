                ...
                <div class="row gx-5">
                    ...
                    <div class="col">
                        <a class="btn btn-secondary float-end ms-1 me-1"
                            href="{{ url('perusahaan/lamar/' . @$loker->id . '/detail') }}" role="button">View All</a>
                        <a class="btn btn-warning float-end ms-1 me-1"
                            href="{{ url('perusahaan/lamar/' . @$loker->id . '/detail/5') }}" role="button">Menunggu</a>
                        <a class="btn btn-primary float-end ms-1 me-1"
                            href="{{ url('perusahaan/lamar/' . @$loker->id . '/detail/1') }}" role="button">Interview</a>
                        <a class="btn btn-success float-end ms-1 me-1"
                            href="{{ url('perusahaan/lamar/' . @$loker->id . '/detail/3') }}" role="button">Diterima</a>
                        <a class="btn btn-danger float-end ms-1 me-1"
                            href="{{ url('perusahaan/lamar/' . @$loker->id . '/detail/2') }}" role="button">Ditolak</a>
                    </div>
                </div>
            </div>
            <div class="bg-secondary card-body">
                        ...
                        <tbody>
                            @foreach ($lamars as $item)
                                <tr class="text-white">
                                    ...
                                    <td><a target="_blank"
                                            href="{{ url('perusahaan/mahasiswa/' . @$item->id_mahasiswa . '/berkas/tampil') }}">Berkas
                                    </td>
                                    </a>
                                    <td
                                        class="{{ $item->lamar_status == 5
                                    ...
                                    <td class="text-center">
                                        @if ($item->lamar_status == 5)
                                            <a class="btn btn-xs btn-success"
                                                href="{{ url('perusahaan/lamar/' . @$item->id . '/status/' . 1) }}"><i
                                                    class="fas fa-check-circle"></i> TERIMA</a>
                                            <a class="btn btn-xs btn-danger ml-2"
                                                href="{{ url('perusahaan/lamar/' . @$item->id . '/status/' . 2) }}"><i
                                                    class="fas fa-times-circle"></i> TOLAK</a>
                                        @endif
                                        @if ($item->lamar_status == 1)
                                            <a class="btn btn-xs btn-success"
                                                href="{{ url('perusahaan/lamar/' . @$item->id . '/status/' . 3) }}"><i
